...
//                $calculated = array(10,20,30);
		foreach ($calculated as $uid => $val) {
			$sql = "SELECT u.* FROM {user} u WHERE u.id = :ueid";
			$userinfo = $DB->get_record_sql($sql, array('ueid' => $uid));
			$username = $userinfo->lastname . ", " . $userinfo->firstname;
			if (array_key_exists($username, $dataset)){
				$dataset[$username] += $val;
			}else{
				$dataset[$username] = $val;
			}
		}
